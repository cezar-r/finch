...
NERDCTL_VERSION=$(echo "$NERDCTL_RELEASE" | sed 's/-.*//')
...
go build -ldflags "-s -w -X ${NERDCTL_PACKAGE}/version.Version=${NERDCTL_VERSION} -X ${NERDCTL_PACKAGE}/version.Revision=${NERDCTL_COMMIT}" -o _output/nerdctl ./cmd/nerdctl
popd
 
# build buildkit
pushd "$TEMP_BUILD_DIR/$BUILDKIT_SRC"
go build -mod=vendor -buildmode=pie -ldflags "-w -X ${BUILDKIT_PACKAGE}/version.Version=${BUILDKIT_VERSION}" -o _output/buildkitd ${BUILDKIT_PACKAGE}/cmd/buildkitd
go build -mod=vendor -buildmode=pie -ldflags "-w -X ${BUILDKIT_PACKAGE}/version.Version=${BUILDKIT_VERSION}" -o _output/buildctl ${BUILDKIT_PACKAGE}/cmd/buildctl
popd
...
for arch in "$AARCH64" "$X86_64"; do
        ARCH_DIR="${SCRIPT_DIR}/finch_${VERSION}_${arch}"
        mkdir -p $ARCH_DIR
        ...
        sudo install -m 0644 "$UNITS/finch.service" "$ARCH_DIR/etc/systemd/system"
        sudo install -m 0644 "$UNITS/finch.yaml" "$ARCH_DIR/etc/finch"
 
        sudo ln -s "$ARCH_DIR/usr/local/bin/finch-daemon" "$ARCH_DIR/usr/libexec/finch/finch-daemon"
        sudo install -m 0644 "$UNITS/finch.socket" "$ARCH_DIR/etc/systemd/system"
...
EOF
        ...
        sudo install -m 0644 "$UNITS/config.toml" "$ARCH_DIR/etc/containerd"
        ...
        sudo install -m 0644 "$UNITS/nerdctl.toml" "$ARCH_DIR/etc/finch/nerdctl"
        ...
        sudo install -m 0644 "$UNITS/buildkitd.toml" "$ARCH_DIR/etc/finch/buildkit"
        sudo install -m 0644 "$UNITS/finch-buildkit.service" "$ARCH_DIR/etc/systemd/system"
        sudo install -m 0644 "$UNITS/finch-buildkit.socket" "$ARCH_DIR/etc/systemd/system"
        ...
        sudo install -m 0644 "$UNITS/finch-soci.service" "$ARCH_DIR/etc/systemd/system"
        sudo install -m 0644 "$UNITS/finch-soci.socket" "$ARCH_DIR/etc/systemd/system"
        sudo install -m 0644 "$UNITS/soci-snapshotter-grpc.toml" "$ARCH_DIR/etc/finch/soci"
...
# build aarch64 deb
dpkg-deb --build --root-owner-group ${SCRIPT_DIR}/finch_${VERSION}_aarch64
 
# build the amd64 deb
dpkg-deb --build --root-owner-group ${SCRIPT_DIR}/finch_${VERSION}_amd64
 
# clean up
sudo rm -rf $SCRIPT_DIR/TMP
sudo find "$SCRIPT_DIR" -name "finch*" ! -name "*.deb" -exec rm -rf {} +
